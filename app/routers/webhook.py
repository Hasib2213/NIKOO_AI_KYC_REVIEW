...
router = APIRouter(prefix="/api/v1", tags=["Sumsub Webhook"])
    ...
    try:
        ...
        if not is_valid:
            logger.warning(
                ...
                f"Received prefix: {signature[:8]}...\n"
                f"Expected prefix: {expected_sig[:8]}...\n"
                f"Algorithm: {alg_header}"
            )
...
async def sumsub_webhook(request: Request):
    ...
    try:
        ...
        logger.info(f"  externalUserId: {external_user_id[:15]}...")
        logger.info(f"  applicantId: {applicant_id[:10]}...")
        logger.info(f"  reviewStatus: {review_status}")
