...
# # No fake/mock data - real liveness verification results from Sumsub API
...
#             is_live=result.get("is_live", False),  # Will be updated by Sumsub webhook
#             confidence=result.get("confidence", 0.0),  # Will be updated by Sumsub webhook
#             checks_passed=result.get("checks_passed", {}),
#             face_detected=bool(result.get("image_id")),  # True if image uploaded
#             message=result.get("message", "Liveness check in progress"),
#             sumsub_data=result  # Return complete Sumsub response
#         )
